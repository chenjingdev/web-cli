...
Backend daemon 재기동 후 in-memory session/snapshot이 비어있어 첫 `agrune_snapshot` 호출이 빈 결과 또는 에러를 반환한다. Content script는 초기 진입 시에만 `session_open`을 보내고, 이후에는 주기적 snapshot만 전송하므로 cold start 직후 복구 경로가 없다.

## 설계 원칙

- **Lazy on-demand**: tool 호출 시점에만 연결/resync 수행. 불필요한 백그라운드 모니터링 없음.
- **Backend 중심 오케스트레이션**: 준비 로직을 `AgagruneBackend.ensureReady()`에 집중.
...
`ResyncRequestMessage` 타입 1개를 `@agrune/core`의 `NativeMessage` union에 추가한다.
...
`AgagruneBackend`의 `handleToolCall()` 최상단에서 `ensureReady()`를 호출한다. 개별 call site가 아닌 `handleToolCall` 내부에서 일괄 처리.
...
5. 3초 내 도착하면 통과
6. 3초 초과 시 에러 반환: `"No browser sessions available. Ensure a page with agrune annotations is open."`

**적용 범위:**

- 적용: `agrune_sessions`, `agrune_snapshot`, `agrune_act`, `agrune_fill`, `agrune_drag`, `agrune_wait`, `agrune_guide` — 모두 동일한 readiness 조건(session + snapshot)을 사용한다. `agrune_sessions`도 snapshot까지 대기하는데, 이는 snapshot이 있어야 유의미한 세션 정보이고 3초 timeout 안에 함께 도착하기 때문이다.
- 제외: `agrune_config` (세션 없이도 설정 가능)
...
**구현 위치:** `agrune-mcp.ts`의 `--backend-daemon` 섹션. `AgagruneBackend`는 라이브러리 클래스이므로 `process.exit()`을 직접 호출하지 않는다.

**메커니즘:**

- `AgagruneBackend`에 `onActivity` 콜백을 추가한다. `handleToolCall()` 진입 시 호출.
- `agrune-mcp.ts`에서 `onActivity` 콜백에 idle 타이머 리셋 로직을 연결한다.
...
```typescript
// agrune-mcp.ts — --backend-daemon 섹션
...
- **Agagrune annotation이 없는 탭:** `init()`에서 `hasAnnotations()` 체크에 의해 content script가 동작하지 않으므로, resync broadcast를 받아도 반응하지 않음. 정상 동작.
...
- 모든 tool(`agrune_sessions`, `agrune_snapshot`, `agrune_act`, `agrune_fill`, `agrune_drag`, `agrune_wait`, `agrune_guide`)이 동일한 준비 게이트를 거친다.
