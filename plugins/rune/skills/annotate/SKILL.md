---
name: annotate
description: webcli 어노테이션을 컴포넌트에 자동으로 추가하는 스킬. 사용자가 "어노테이션 달아줘", "이 컴포넌트를 AI가 제어할 수 있게 해줘", "webcli 어노테이션", "annotate", "data-webcli 추가", "이 페이지 전체에 어노테이션", "프로젝트 전체 어노테이션" 등을 말하면 반드시 이 스킬을 사용할 것. 컴포넌트나 페이지의 인터랙티브 요소를 분석하고 적절한 data-rune-* 속성을 제안/적용하는 모든 작업에 이 스킬이 필요하다.
---

# webcli Annotate

컴포넌트의 인터랙티브 요소를 분석하여 `data-rune-*` 어노테이션을 자동으로 추가한다.
...
| 속성 | 설명 | 정적/동적 |
|------|------|-----------|
| `data-rune-action` | `"click"` 또는 `"fill"` | **정적만 가능** |
| `data-rune-name` | 타겟의 표시 이름 | 정적 또는 동적 |
| `data-rune-desc` | 타겟이 하는 일 설명 | 정적 또는 동적 |

### 선택 속성

| 속성 | 설명 |
|------|------|
| `data-rune-key` | 명시적 타겟 ID (정적만 가능) |
| `data-rune-group` | 그룹 ID (여러 타겟을 묶음) |
| `data-rune-group-name` | 그룹 표시 이름 |
| `data-rune-group-desc` | 그룹 설명 |
| `data-rune-sensitive` | `"true"` 시 값 미리보기 숨김 (비밀번호 등) |
...
{columns.map(column => (
  <div
    key={column.id}
    data-rune-group={`column-${column.id}`}
    data-rune-group-name={column.title}
    data-rune-group-desc={`${column.title} 컬럼의 카드 목록`}
  >
    <h3>{column.title}</h3>
    ...
    <div
      ...
      data-rune-name={`${column.title} 컬럼`}
      data-rune-desc="이 컬럼으로 카드를 이동"
    >
      {column.tasks.length === 0 ? (
        ...
        <div
          ...
          data-rune-action="click"
          data-rune-name={`${column.title} 빈 영역`}
          data-rune-desc="빈 컬럼에 카드를 놓기"
        >
          No tasks
        </div>
      ) : (
        column.tasks.map(task => (
          <div key={task.id} className="card">
            ...
            <button
              ...
              data-rune-action="click"
              data-rune-name={task.title}
              data-rune-desc="이 카드를 드래그하여 이동"
            >
              ⠿
            </button>
...
<Select>
  <SelectTrigger
    data-rune-action="click"
    data-rune-name="역할 필터"
    data-rune-desc="역할별 필터 드롭다운 열기"
  >
    <SelectValue placeholder="Role" />
  </SelectTrigger>
  <SelectContent
    data-rune-group="role-filter-options"
    data-rune-group-name="역할 필터 옵션"
    data-rune-group-desc="역할 필터 드롭다운의 선택지"
  >
    <SelectItem value="all"
      data-rune-action="click"
      data-rune-name="All Roles"
      data-rune-desc="모든 역할의 멤버 표시"
    >All Roles</SelectItem>
    <SelectItem value="admin"
      data-rune-action="click"
      data-rune-name="Admin"
      data-rune-desc="Admin 역할 멤버만 필터링"
    >Admin</SelectItem>
  </SelectContent>
</Select>
```
...
{members.map(member => (
  <tr data-rune-group={`member-${member.id}`}>
    <td>
      <button
        data-rune-action="click"
        data-rune-name={member.name}
        data-rune-desc="멤버 프로필 보기"
      >
        {member.name}
      </button>
    </td>
    <td>
      <button
        data-rune-action="click"
        data-rune-name={`${member.name} 삭제`}
        data-rune-desc="멤버를 목록에서 제거"
      >
        삭제
      </button>
    </td>
...
<nav
  data-rune-group="main-nav"
  data-rune-group-name="메인 네비게이션"
  data-rune-group-desc="페이지 간 이동 메뉴"
>
  <button data-rune-action="click" data-rune-name="홈" data-rune-desc="홈 페이지로 이동">홈</button>
  <button data-rune-action="click" data-rune-name="설정" data-rune-desc="설정 페이지로 이동">설정</button>
</nav>
```
...
```tsx
<DialogContent data-rune-group="task-wizard" data-rune-group-name="태스크 생성 위자드">
  {step === 0 && (
    <div data-rune-group="wizard-step-basic" data-rune-group-name="기본 정보">
      <Input data-rune-action="fill" data-rune-name="태스크 이름" data-rune-desc="새 태스크의 제목 입력" />
      {/* Select + 모든 SelectItem에 어노테이션 */}
    </div>
  )}
  <div data-rune-group="wizard-navigation" data-rune-group-name="위자드 네비게이션">
    <Button data-rune-action="click" data-rune-name="이전 단계" data-rune-desc="이전 입력 단계로 돌아가기" />
    <Button data-rune-action="click" data-rune-name="다음 단계" data-rune-desc="다음 입력 단계로 진행" />
  </div>
</DialogContent>
```
...
```ts
// vite.config.ts
import webCliDomPlugin from '@runeai/build-core'
...
```ts
// main.ts 또는 main.tsx
import '@runeai/build-core/register'
```
