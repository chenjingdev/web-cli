---
name: annotate
description: agrune 어노테이션을 컴포넌트에 자동으로 추가하는 스킬. 사용자가 "어노테이션 달아줘", "이 컴포넌트를 AI가 제어할 수 있게 해줘", "agrune 어노테이션", "annotate", "data-agrune 추가", "이 페이지 전체에 어노테이션", "프로젝트 전체 어노테이션" 등을 말하면 반드시 이 스킬을 사용할 것. 컴포넌트나 페이지의 인터랙티브 요소를 분석하고 적절한 data-agrune-* 속성을 제안/적용하는 모든 작업에 이 스킬이 필요하다.
---

# agrune Annotate

컴포넌트의 인터랙티브 요소를 분석하여 `data-agrune-*` 어노테이션을 자동으로 추가한다.
...
| 속성 | 설명 | 정적/동적 |
|------|------|-----------|
| `data-agrune-action` | `"click"` 또는 `"fill"` | **정적만 가능** |
...
| `data-agrune-sensitive` | `"true"` 시 값 미리보기 숨김 (비밀번호 등) |
...
>
  ...
  <button data-agrune-action="click" data-agrune-name="설정" data-agrune-desc="설정 페이지로 이동">설정</button>
</nav>
```
...
<DialogContent data-agrune-group="task-wizard" data-agrune-group-name="태스크 생성 위자드">
  {step === 0 && (
    <div data-agrune-group="wizard-step-basic" data-agrune-group-name="기본 정보">
      <Input data-agrune-action="fill" data-agrune-name="태스크 이름" data-agrune-desc="새 태스크의 제목 입력" />
      {/* Select + 모든 SelectItem에 어노테이션 */}
    </div>
  )}
  <div data-agrune-group="wizard-navigation" data-agrune-group-name="위자드 네비게이션">
    <Button data-agrune-action="click" data-agrune-name="이전 단계" data-agrune-desc="이전 입력 단계로 돌아가기" />
    <Button data-agrune-action="click" data-agrune-name="다음 단계" data-agrune-desc="다음 입력 단계로 진행" />
  </div>
</DialogContent>
```
...
```ts
// vite.config.ts
import agruneDomPlugin from '@agagrune/build-core'
