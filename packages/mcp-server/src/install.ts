...
export async function runInstall(options?: { extensionId?: string }): Promise<void> {
  ...
  // 3. Copy extension to stable location (~/.runeai/extension/)
  console.log('2. Installing extension to ~/.runeai/extension/...')
  ...
  const entryJs = join(WEBCLI_HOME, 'mcp-server/bin/webcli-mcp.js')
  writeFileSync(
    wrapperPath,
    [
      '#!/bin/sh',
      '# rune native messaging host',
