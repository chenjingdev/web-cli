...
import { encodeMessage, decodeMessages } from '@agrune/browser'

describe('Integration: MCP server end-to-end', () => {
  it('receives session_open and creates session', async () => {
    const { driver, connectNativeMessaging } = createMcpServer()
    ...
    expect(driver.sessions.getSessions()).toHaveLength(1)
    expect(driver.sessions.getSession(42)?.url).toBe('http://test.com')
  })
  ...
  it('receives snapshot_update and caches it', async () => {
    ...
    expect(driver.sessions.getSnapshot(42)).toEqual(snapshot)
  })
  ...
  it('sends command_request and resolves on result', async () => {
    ...
    // Enqueue a command
    const resultPromise = driver.commands.enqueue(42, { kind: 'act', targetId: 'btn-1' })
    await new Promise(r => setTimeout(r, 50))
  ...
  it('closes session on session_close', async () => {
    ...
    await new Promise(r => setTimeout(r, 50))
    expect(driver.sessions.getSessions()).toHaveLength(0)
  })
  ...
  it('responds to get_status with current session and recent agent activity state', async () => {
    const { driver, handleToolCall, connectNativeMessaging } = createMcpServer()
    const fakeInput = new Readable({ read() {} })
    const outputChunks: Buffer[] = []
    const fakeOutput = new Writable({
      write(chunk, _enc, cb) { outputChunks.push(chunk); cb() },
    ...
    await handleToolCall('agrune_sessions', {})

    fakeInput.push(encodeMessage({ type: 'get_status' } as any))
    await new Promise(r => setTimeout(r, 50))

    const { messages } = decodeMessages(Buffer.concat(outputChunks))
    const statusMsg = messages.find((m: any) => m.type === 'status_response') as any
    expect(statusMsg).toBeDefined()
    expect(statusMsg.status.hostName).toBe('com.agrune.agrune')
    expect(statusMsg.status.phase).toBe('connected')
    expect(statusMsg.status.connected).toBe(true)
    expect(statusMsg.status.lastError).toBeNull()
    expect(statusMsg.status.sessionCount).toBe(1)
    // mcpConnected is determined by activity blocks in ExtensionDriver;
    // agrune_sessions does not push activity blocks (only command execution does)
  })
})
