import type { NativeMessage, AgruneRuntimeConfig, NativeHostStatus } from '@agrune/core'
import type { CdpHandler } from './cdp-handler.js'
import type { NativeHostController } from './native-host-controller.js'

// --- Types inlined from extension shared/messages ---
...
export type ExtensionMessage =
  ...
  | GetNativeHostStatusMessage
  | ReconnectNativeHostMessage
  | { type: 'native_host_status_changed'; status: NativeHostStatus }

// --- Types inlined from extension tab-broadcast ---
...
// --- Config persistence (default: noop, extension sets real one) ---

type PersistConfig = (partial: Partial<AgruneRuntimeConfig>) => Promise<AgruneRuntimeConfig>
...
export function createBackgroundMessageRouter(options: BackgroundMessageRouterOptions): BackgroundMessageRouter {
  ...
  const defaultPersist: PersistConfig = async (partial) => partial as AgruneRuntimeConfig
  const persistConfig = options.persistConfig ?? defaultPersist
